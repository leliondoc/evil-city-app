...
Les paramètres proposent le volume des effets, une coupure du son et un bouton de test. Les préférences sont mémorisées dans ce navigateur. Le son démarre après une interaction, s’arrête pendant la pause ou lorsque l’onglet est masqué, et accompagne les récoltes actives, livraisons, combats, recrutements terminés et bâtiments terminés. Les actions hors champ sont silencieuses ; les sons simultanés sont plafonnés à quatre, avec un délai entre effets similaires. Voir `ASSETS.md` pour importer le pack gratuit TomMusic avant l’essai local.
