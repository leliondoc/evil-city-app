...
export function humanBuildingDescription(kind: BuildingKind, level: number) {
  ...
  const role = kind === 'hall'
    ? 'Centre de commandement de la garde municipale. Un chevalier, un lancier, une archère et un moine d’élite défendent la mairie et sortent dans la rue au premier assaut. Les trolls peuvent soutenir un assaut prolongé.'
    : kind === 'guild'
