...
await page.evaluate(()=>{const l=window.manorState.lots[6];l.kind='den';l.level=2;});
await clickSelection('lot',6);
const benefit=page.locator('.upgrade-benefit');
await benefit.waitFor();await page.waitForFunction(()=>document.querySelector('.upgrade-benefit')?.textContent.includes('Population maximale'));
assert.match(await benefit.innerText(),/24 places[\s\S]*30 places/);
await page.locator('.upgrade-benefit-values').scrollIntoViewIfNeeded();
const values=await page.locator('.upgrade-benefit-values').boundingBox();const panel=await page.locator('.selection-scroll').boundingBox();
assert.ok(values.x>=panel.x&&values.x+values.width<=panel.x+panel.width);
await page.screenshot({path:join(tmpdir(),'evil-upgrade-preview-'+viewport.width+'.png')});
await page.close();console.log(viewport,'global army preview includes both caves and stays inside the panel');
