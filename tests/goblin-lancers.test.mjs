...
test('The catalog contains all playable creatures and every human role; production art uses the cave and Troll House', () => {
  ...
  const lancer = BESTIARY_CREATURES.find((e) => e.id === 'spear-goblin');
  assert.equal(lancer.evolution.name, 'Chevaucheur de cochon');
  assert.ok(!BESTIARY_CREATURES.some((e) => e.id === 'pig-rider'));
