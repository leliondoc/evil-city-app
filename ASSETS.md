...
Les arbres, les buissons et la fumée de la tanière utilisent également les animations originales. La mairie utilise le château bleu, le manoir le château violet, la hutte des trolls (identifiant interne `forge`) la Troll House (`Extra/Dead Tree/Dead Tree.png`) et la crypte le monastère violet. Les rues pavées sont une géométrie de terrain simple dessinée par le moteur.
...
Les cinq composants de `components/ui/` dérivent de shadcn/ui sous licence MIT (copyright 2023 shadcn). Ils utilisent Base UI et les variantes natives Tailwind. La mention complète est distribuée dans `public/licenses/shadcn-MIT.txt`. Les composants non utilisés et le générateur shadcn ont été retirés lors de l’audit du 12 septembre 2026.

Les réparations exigent un ouvrier à l’entrée du bâtiment (4 PV/s après 5 s sans ennemi proche). Un gobelin disponible répare les bâtiments du joueur ; un paysan disponible répare les bâtiments humains. Les chantiers de ruines restent distincts. Les animations humaines originales `Units/Blue Units/Pawn/Pawn_Interact Hammer.png` et `Pawn_Run Hammer.png` du Free Pack sont copiées sans retouche dans `pawn-hammer-interact.png` et `pawn-hammer-run.png` (192 px, 3 et 6 images). Les gobelins utilisent les effets de chantier existants du pack.
